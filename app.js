...
container.resolve(function (register, _, home ,admin, results, group, landing, travel){
    ...
    //        console.dir(error);
    // });
    mongoose.connect('mongodb://127.0.0.1/Ahero',{useMongoClient: true}, function(error, db) {
        if(!error){
             console.log("We are connected");
        }
        else
...
// );
    ...
    function SetupExpress() {
        ...
        register.SetRouting(router);
        admin.SetRouting(router);
        home.SetRouting(router);
        group.SetRouting(router);
        results.SetRouting(router);
        landing.SetRouting(router);
        travel.SetRouting(router);
